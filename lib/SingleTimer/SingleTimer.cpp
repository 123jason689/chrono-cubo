...
SingleTimer::SingleTimer(Adafruit_SSD1306* displayInstance) 
    : display(displayInstance), isRunning(false), isFinished(false), startTime(0), duration(0), remainingTime(0),
      setupMinutes(0), setupSeconds(0), editingMinutes(true), lastDisplayUpdate(0) {
}
...
void SingleTimer::handleSetupInput() {
    // Handle Y-axis movement to switch between minutes and seconds
    if (can_move()) {
    ...
    if (can_move()) {
        ...
        if (x_move != 0) {
            if (editingMinutes) {
                if (x_move == 1) {
                    setupMinutes = (setupMinutes < 99) ? setupMinutes + 1 : 0;
                } else {
                    setupMinutes = (setupMinutes > 0) ? setupMinutes - 1 : 99;
                }
            } else {
                if (x_move == 1) {
                    setupSeconds = (setupSeconds < 59) ? setupSeconds + 1 : 0;
                } else {
                    setupSeconds = (setupSeconds > 0) ? setupSeconds - 1 : 59;
                }
            }
            drawSetupScreen();
        }
...
void SingleTimer::updateTimer() {
    ...
    if (remainingTime <= 0) {
        ...
        notificationManager.playAlert(1, 25);
        drawFinishedScreen();
        return;
    }
...
void SingleTimer::drawSetupScreen() {
    ...
    display->println("X-Pot: Adjust value");
    display->println("Button: Start timer");
